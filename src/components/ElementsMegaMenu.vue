      ...
      <div class="bg-surface border-t border-outline-variant/20 shadow-2xl">
        <div class="max-w-5xl mx-auto px-6 lg:px-8 py-10">
          ...
          <div class="grid grid-cols-3 gap-0 border-collapse">
            <div v-for="el in elements" :key="el.symbol"
              @click="go(el.route)"
              @keydown.enter="go(el.route)"
              @keydown.space.prevent="go(el.route)"
              tabindex="0" role="menuitem"
              class="p-6 cursor-pointer bg-surface-container-lowest hover:bg-tertiary transition-colors duration-300 group text-center"
            >
              <div class="inline-block border border-on-surface/20 group-hover:border-white/30 p-3 mb-3">
              ...
              <p class="font-body text-xs text-on-surface-variant group-hover:text-white/70 mt-2 leading-relaxed max-w-[200px] mx-auto">{{ el.short }}</p>
            </div>
          </div>
        </div>
      </div>
